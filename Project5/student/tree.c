#include "tree.h"
Node *find_Helper(Node *node, int id);
int id_counter = 0;
...
Node *create_node(const char *str) {
  ...
  id_counter++;
  newNode -> data = dynamic_string_copy(str);
...
Node *find_node(Tree *tree, int id) {
  ...
  if (id < 0) return NULL;
  return find_Helper(tree -> root, id);
}

Node *find_Helper(Node *node, int id) {
  if (node -> id == id) return node;
  else if (node -> num_children == 0) return NULL;
  else {
    for (int i = 0; i < node -> num_children; i++) {
      Node *ret = find_Helper(node -> children[i], id);
...
char *update_node(Tree *tree, int id, const char *new_data) {
  if (tree == NULL || new_data == NULL  ) return NULL;
  Node *target = find_node(tree, id);
  if (target == NULL) return NULL;
  char *oldData = target -> data;
  char *malloced = malloc((strlen(new_data) + 1) * sizeof(char));
  if (malloced == NULL) return NULL;
  strcpy(malloced, new_data);
  target -> data = malloced;
  return oldData;
}
...
int insert_node(Tree *tree, int parent_id, const char *new_child_data) {
  ...
  if (tree == NULL || parent_id < 0 || new_child_data == NULL) return 0;
  Node *parent = find_node(tree, parent_id);
  if (parent == NULL) return 0;
  Node *child = create_node(new_child_data);
  if (child == NULL) return 0;
  Node **children = realloc(parent -> children, (parent->num_children + 1) * sizeof(Node));
  if (children == NULL) {
    free(child -> data);
    free(child);
    return 0;
